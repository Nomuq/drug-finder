...
@interface LactMedSearchViewController ()
@property (nonatomic, strong) DBManager *dbManager;

@property (nonatomic, strong) NSArray *arrqueryresult;
@end

@implementation LactMedSearchViewController
...
- (IBAction)act_search:(id)sender {
    if([_txt_search.text length] > 2)
    {
        ...
        NSString *query =[NSString stringWithFormat:@"select * from lactmed where SubstanceName LIKE '%@'", str];
        //NSLog(@"%@", query);
        // Get the results.
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSInteger indexOftitle = [self.dbManager.arrColumnNames indexOfObject:@"SubstanceName"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    LactMedTableViewController *update = [self.storyboard instantiateViewControllerWithIdentifier:@"lactmedtable"];
