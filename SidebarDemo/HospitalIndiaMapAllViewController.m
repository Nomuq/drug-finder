...
#import "DBManager.h"
@interface HospitalIndiaMapAllViewController ()
...
CLLocation *currentLocation12hi;
@implementation HospitalIndiaMapAllViewController
...
- (void)viewDidLoad {
    ...
    if (self.loc1 != nil) {
        currentLocation12hi = self.loc1;
        //closestLocation = self.locclosest;
    }
    CLLocation *currentLocation4 = [[CLLocation alloc]initWithLatitude:currentLocation12hi.coordinate.latitude longitude:currentLocation12hi.coordinate.longitude];
    
    // coordinate -33.86,151.20 at zoom level 6.
    GMSCameraPosition *camera = [GMSCameraPosition cameraWithLatitude:currentLocation12hi.coordinate.latitude
                                                            longitude:currentLocation12hi.coordinate.longitude
                                                                 zoom:12];
    ...
    marker2.position = CLLocationCoordinate2DMake(currentLocation12hi.coordinate.latitude, currentLocation12hi.coordinate.longitude);
    ...
    NSLog(@"%@", currentLocation4);
        ...
        NSString *query =[NSString stringWithFormat:@"select * from hospital"];
        ...
        for (int i=0; i<self.arrqueryresult.count; i++) {
            ...
            //NSLog(@"%f", latitu2);
            CLLocation *currentLocation12hi = [[CLLocation alloc]initWithLatitude:latitu2 longitude:longti2];
            [mut addObject:currentLocation12hi];
           ...
           NSString *addressofbb3 = [NSString stringWithFormat:@"%@\n%@",[[self.arrqueryresult objectAtIndex:0] objectAtIndex:indexOfbbaddress],[[self.arrqueryresult objectAtIndex:i] objectAtIndex:indexOfbbpincode]];
...
- (void)locationManager:(CLLocationManager *)manager didUpdateToLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation
{
    ...
    currentLocation12hi = newLocation;
    
    if (currentLocation12hi != nil) {
        //NSLog(@"%@",[NSString stringWithFormat:@"%.8f", currentLocation12hi.coordinate.latitude]);
        //NSLog(@"%@",[NSString stringWithFormat:@"%.8f", currentLocation12hi.coordinate.longitude]);
    }
}
